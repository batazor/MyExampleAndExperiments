...
func TestCostValues(t *testing.T) {
	is := is.New(t)
	is.Equal(int(meander.Cost1), "$")
	is.Equal(int(meander.Cost2), "$$")
	is.Equal(int(meander.Cost3), "$$$")
	is.Equal(int(meander.Cost4), "$$$$")
	is.Equal(int(meander.Cost5), "$$$$$")
}
